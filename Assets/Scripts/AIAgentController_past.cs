...
public class AIAgentController_past : Agent
{
    [Header("オブジェクト参照")]
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        //相対座標
        Vector3 relativePlayerPos = playerPos - selfPos;
        Vector3 relativePuckPos = puckPos - selfPos;

        currentObservation[0] = selfPos.x * dir;
        currentObservation[1] = selfPos.z * dir;
        currentObservation[2] = relativePlayerPos.x * dir; // プレイヤーとの相対位置
        currentObservation[3] = relativePlayerPos.z * dir; // プレイヤーとの相対位置
        currentObservation[4] = relativePuckPos.x * dir; // パックとの相対位置
        currentObservation[5] = relativePuckPos.z * dir; // パックとの相対位置
        currentObservation[6] = puckVel.x * dir; // パックの速度
        currentObservation[7] = puckVel.z * dir; // パックの速度
